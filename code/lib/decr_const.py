...
class DecrAPSPConstTAlgo(DynAlgo):
...
if __name__ == "__main__":
    ...
    dyn_apsp_algo = DecrAPSPConstTAlgo(base_graph,epsilon,c)
    ...
    while True:
        total = 0
        fails = 0
        uv_s = list(dyn_apsp_algo.base_graph.edges)
        if(len(uv_s) < 100):
            break
        if not first_time:
            ...
            for _ in tqdm(range(random.randint(10,100))):
                ...
                uv_s.remove(uv)
                dyn_apsp_algo.base_graph.remove_edge(*uv)
                if not nx.is_connected(dyn_apsp_algo.base_graph):
                    skip = True
                dyn_apsp_algo.base_graph.add_edge(*uv)
                if not skip:
                    dyn_apsp_algo.delete(*uv)
                    n_deletions += 1
        ...
        for u,v in tqdm(list(combinations(dyn_apsp_algo.base_graph.nodes,2))):
            try: 
                ...
                continue
            given_ans = dyn_apsp_algo.query(u,v,true_ans)
